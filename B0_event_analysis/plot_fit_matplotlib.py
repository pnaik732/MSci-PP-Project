...
def Fitting(variable, fit_func, weight, p0_list,bin):
    ...
    return popt, np.sqrt(np.diag(pcov)), variable_fit
...
def OneD_histo(ax,variable, weight, fit_func, title, label0, xlabel, style, histtype, p0_list, bin):
	...
	popt, perr, variable_fit = Fitting(variable, fit_func,weight, p0_list,bin)
...
def main (program,type,event,phase,event_n):
	...
	bin=100
	#f0,axs0 = plt.subplots(2,2,figsize=(10,10))

	# (ax0_1, ax0_2), (ax0_3, ax0_4) = axs0
	# B0_p_popt,      B0_p_err      ,_ =  Fitting(np.array(B0_p),      Gaussian, B0_weight_p,[1,1,1],([0,0,0],[max()]))
	...
	mKpi_p_popt,      mKpi_p_err      ,_ =  Fitting(np.array(mKpi_p),      BreitWigner, np.ones(len(mKpi_p)),      p1_list,bin)
	...
	mDDbar_p_popt,      mDDbar_p_err      ,_ =  Fitting(np.array(mDDbar_p),      BreitWigner3, np.ones(len(mDDbar_p)),     p2_list,bin)

...
def main(program,name):   
	...
	sWeight     = CM_data[6];            sWeight_conj     = CM_data_conj[6]
	...
	h0_1 = pltini("h0_1",bin,mDDbar,sWeight,'\ m(D^0\\bar{D}^0)\\;[GeV/c^2]',ROOT.kBlack)
	...
	h0_2 = pltini("h0_2",bin,mDDbar_conj,sWeight_conj,'\ m(\\bar{D}^0 D^0)\\;[GeV/c^2]',ROOT.kBlack)
	...
	c0.SaveAs("results_%s/invmass_D0Dbar0_fit.png"%(name))

	c00 = ROOT.TCanvas("c00", "Bbar0 decay invmass", 1420,710); c00.Divide(2,1) 
	c00.cd(1); gPad.SetMargin(0.2,0.05,0.15,0.1)
	h00_1 = pltini("h00_1",bin,mKpi, sWeight, '\ m(K^+\pi^-)\\;[GeV/c^2]',     ROOT.kBlack)
	...
	h00_2 = pltini("h00_2",bin,mKpi_conj,sWeight_conj,  '\ m(K^-\pi^+)\\;[GeV/c^2]',     ROOT.kBlack)
	...
	c00.SaveAs("results_%s/invmass_Kpi_fit.png"%(name))

	with open('results_%s/invmass_fit_parameters.txt'%(name),mode='a') as f:
